...
# from od.box_coders import faster_rcnn_box_coder
# from od.box_coders import mean_stddev_box_coder
# from od.core import box_coder as bcoder
# from od.core import matcher as mat
# from od.core import region_similarity_calculator as sim_calc
# from od.core import standard_fields as fields
# from od.core.matchers import argmax_matcher
...
#     def assign(self, anchors, groundtruth_boxes, unmatched_class_label=None):
...
#         :param anchors: a BoxList representing N anchors
#         :param groundtruth_boxes: a BoxList representing M groundtruth boxes
#         :param groundtruth_labels:  a tensor of shape [M, d_1, ... d_k]
#             with labels for each of the ground_truth boxes. The subshape
#             [d_1, ... d_k] can be empty (corresponding to scalar inputs).  When set
#             to None, groundtruth_labels assumes a binary problem where all
#             ground_truth boxes get a positive label (of 1).
#         :param unmatched_class_label: a float32 tensor with shape [d_1, d_2, ..., d_k]
#             which is consistent with the classification target for each
#             anchor (and can be empty for scalar targets).  This shape must thus be
#             compatible with the groundtruth labels that are passed to the "assign"
#             function (which have shape [num_gt_boxes, d_1, d_2, ..., d_k]).
#             If set to None, unmatched_cls_target is set to be [0] for each anchor.
#         :param groundtruth_weights: a float tensor of shape [M] indicating the weight to
...
#             tf.shape(unmatched_class_label))
#         labels_and_box_shapes_assert = shape_utils.assert_shape_equal(
#             tf.shape(groundtruth_labels)[:1],
#             tf.shape(groundtruth_boxes.get())[:1])
...
#     for anchors, gt_boxes, gt_class_targets, gt_weights in zip(anchors_batch, gt_box_batch,
#                                                                gt_class_targets_batch,
#                                                                gt_weights_batch):
#         (cls_targets, cls_weights, reg_targets, reg_weights,
#          match) = target_assigner.assign(anchors, gt_boxes, gt_class_targets, unmatched_class_label,
#                                          gt_weights)
